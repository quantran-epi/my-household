...
const emptyPersonalSlice = (partKey: PersonalPartKey): unknown => {
    ...
    if (partKey === "cookingSession") return { sessions: [], cookTimeStats: {}, dishFeedback: {}, dishFeedbackHistory: [] };
    if (partKey === "householdHealth") return { profiles: {}, records: [] };
    return {};
};

const normalizePersonalSlice = (partKey: PersonalPartKey, value: unknown): unknown => {
    const fallback = emptyPersonalSlice(partKey);
    if (
...
const readChangedRemoteSlices = async (params: {
    ...
    await Promise.all(personalPartKeys.map(async partKey => {
        ...
        restoredSlices[partKey] = normalizePersonalSlice(partKey, JSON.parse(file.content));
        fetchedCount += 1;
    }));
...
export const useGistBackup = (): UseGistBackupResult => {
    ...
    const personalSlices: PersonalSlices = useMemo(() => ({
        ...
        cookingSession: { sessions: cookingSessions, cookTimeStats, dishFeedback, dishFeedbackHistory },
        householdHealth,
    }), [appContext, inventory, shoppingLists, scheduledMeals, selectedMeals, cookingSessions, cookTimeStats, dishFeedback, dishFeedbackHistory, householdHealth]);
