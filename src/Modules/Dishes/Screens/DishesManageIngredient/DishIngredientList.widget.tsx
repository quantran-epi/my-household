import { DeleteOutlined, QuestionCircleOutlined, EditOutlined, FireOutlined, MoreOutlined } from "@ant-design/icons"
...
export const IngredientItem: React.FunctionComponent<IngredientItemProps> = (props) => {
    ...
    const actions = [
        <ActionButton aria-label="Sửa" icon={<EditOutlined />} shape="circle" onClick={_onEdit} />,
        <Popconfirm title="Xóa nguyên liệu này khỏi món?" onConfirm={() => props.onDelete(props.dish, props.ingredientAmount)}>
            <ActionButton tone="danger" aria-label="Xóa" icon={<DeleteOutlined />} shape="circle" />
        </Popconfirm>
    ]

    if (props.ingredientAmount.prepare?.length > 0) actions.push(<Popover title={<Space>
        ...
        <Button type="text" aria-label="Sơ chế" icon={<MoreOutlined />} style={{ width: 32, paddingInline: 0 }} />
    </Popover>);
