import { CheckCircleOutlined, DeleteOutlined, FireOutlined, MoreOutlined, PlusOutlined, RestOutlined, RollbackOutlined, SearchOutlined } from '@ant-design/icons';
...
const LeftoverItemRow: React.FC<{
    ...
    const slot = item.mealSlot ? MealFeedbackTitleHelper.getSlotLabel(item.mealSlot) : null;
    const mealName = MealFeedbackTitleHelper.normalize(item.mealTitle, item.mealSlot);
    const mealDate = item.mealDate ? dayjs(item.mealDate).format('DD/MM/YYYY') : null;
    const mealLine = [slot, mealName && mealName !== slot ? mealName : null, mealDate].filter(Boolean).join(' · ');
    const eatByLabel = item.eatBy ? `ăn trước ${dayjs(item.eatBy).format('DD/MM')}` : 'ăn khi tiện';
    const actionMenuItems = item.status === 'available' ? [
        { label: 'Ăn một phần', key: 'eat-part', icon: <RollbackOutlined /> },
    ...
    return <Box style={{ width: '100%', boxSizing: 'border-box', borderRadius: 8, border: `1px solid ${visual.border}`, background: '#fff', overflow: 'hidden', boxShadow: '0 6px 16px rgba(15,23,42,0.05)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '5px minmax(0, 1fr)', minWidth: 0 }}>
            ...
            <div style={{ padding: '11px 12px', minWidth: 0 }}>
                <div style={{ display: 'grid', gridTemplateColumns: item.status === 'available' ? 'minmax(0, 1fr) auto' : 'minmax(0, 1fr)', gap: 8, alignItems: 'flex-start' }}>
                    ...
                    {item.status === 'available' && <Dropdown
                        trigger={['click']}
                        placement='bottomRight'
                        menu={{
                            items: actionMenuItems,
                            onClick: ({ key }) => {
                                if (key === 'eat-part') onEatPart();
                                if (key === 'finish') onFinish();
                                if (key === 'discard') onDiscard();
                            },
                        }}
                    >
                        <Button aria-label='Thao tác phần còn lại' type='text' icon={<MoreOutlined />} style={{ width: 32, height: 32, paddingInline: 0, borderRadius: 999, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }} />
                    </Dropdown>}
                </div>
                <Stack align='center' gap={4} wrap='wrap' style={{ marginTop: 10 }}>
...
export const LeftoverManagementScreen: React.FC = () => {
    ...
    const filtered = useMemo(() => {
        ...
        return items.filter(item => {
            ...
            const haystack = [item.dishName, item.note, MealFeedbackTitleHelper.normalize(item.mealTitle, item.mealSlot), item.mealSlot ? MealFeedbackTitleHelper.getSlotLabel(item.mealSlot) : ''].filter(Boolean).join(' ').toLowerCase();
    ...
    usePageActions([
        { key: 'add-leftover', label: 'Thêm phần còn lại', icon: <PlusOutlined />, onClick: _openAddModal },
    ], [_openAddModal]);
    ...
    return <Box style={{ width: 'min(920px, calc(100vw - 24px))', margin: '0 auto', padding: '0 0 96px' }}>
        <Box style={{ border: '1px solid rgba(56,158,13,0.18)', borderRadius: 8, background: 'linear-gradient(135deg, #ffffff 0%, #f6ffed 100%)', padding: 12, marginBottom: 12, boxShadow: '0 10px 26px rgba(15,23,42,0.07)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 8, marginTop: 12 }}>
                ...
                <Box style={{ border: '1px solid rgba(15,23,42,0.06)', borderRadius: 8, background: '#fff', padding: 10 }}>
                    <Typography.Text type='secondary' style={{ display: 'block', fontSize: 11, lineHeight: '15px' }}>Đã lưu</Typography.Text>
                    <Typography.Text strong style={{ display: 'block', color: '#1677ff', fontSize: 20, lineHeight: '26px' }}>{stats.totalCount}</Typography.Text>
                    <Typography.Text type='secondary' style={{ display: 'block', fontSize: 11, lineHeight: '15px' }}>tất cả trạng thái</Typography.Text>
                </Box>
            </div>
        </Box>
        ...
        >
            {eatPartItem && <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div>
                    <Typography.Text strong style={{ display: 'block', fontSize: 12, marginBottom: 5 }}>Số phần đã ăn (phần)</Typography.Text>
                    <NumberStepper
                        min={0.5}
                        step={0.5}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(140px, 170px)', gap: 8 }}>
                    <div>
                        ...
                        <NumberStepper min={0.5} step={0.5} max={99} value={addForm.portions} onChange={value => _updateAddForm({ portions: Number(value ?? 0) })} style={{ width: '100%' }} />
                    </div>
                    <div>
