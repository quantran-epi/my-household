...
import { DatePicker, Empty, Select, Segmented, Spin, Switch } from 'antd';
...
export const SmartMealPlannerScreen: React.FC = () => {
    ...
    const PlannerFieldLabel = ({ helpKey, label, children }: { helpKey: string; label: React.ReactNode; children: React.ReactNode }) => <>
        <div className='smart-planner-field-label'>
            ...
            <ActionButton shape='circle' aria-label={`Giải thích ${String(label)}`} icon={<QuestionCircleOutlined />} onClick={() => _toggleHelp(helpKey)} style={{ color: openHelpKey === helpKey ? '#13a8a8' : '#6b7280' }} />
        </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '46px minmax(0, 1fr) auto', gap: 9, alignItems: 'start' }}>
                ...
                <div style={{ minWidth: 0 }}>
                    ...
                    <div className='smart-planner-slot-facts'>
                        ...
                        {item.leftoverSource && _renderSlotFact(<RestOutlined />, `${item.leftoverSource.eatByLabel} · còn ${item.leftoverSource.portions} phần`)}
                    </div>
    ...
    const CookNowRecommendationCard = ({ item, category }: { item?: SmartPlannerDishRecommendation; category?: SmartPlannerCookNowCategory }) => {
        ...
        return <Box className='smart-planner-result-card'>
            ...
            <div className='smart-planner-result-actions'>
                <ActionButton tone='success' className='smart-planner-result-primary' icon={<PlayCircleOutlined />} onClick={() => _confirmStartCookingRecommendation(item)}>Nấu</ActionButton>
                <Dropdown
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, width: '100%' }}>
                    {detailSelection.item.scoreDetails.map(detail => {
                        ...
                        return <Box key={`${detail.label}-${detail.value}`} style={{ width: '100%', boxSizing: 'border-box', border: '1px solid rgba(15,23,42,0.07)', borderRadius: 8, background: '#f8fafc', padding: methodology ? '10px 44px 10px 10px' : 10, position: 'relative' }}>
                            {methodology && <ActionButton shape='circle' aria-label={`Cách tính ${detail.label}`} icon={<QuestionCircleOutlined />} onClick={() => _toggleHelp(methodologyKey)} style={{ position: 'absolute', top: 8, right: 8, color: openHelpKey === methodologyKey ? '#13a8a8' : '#9ca3af' }} />}
    ...
    const renderStepperControl = (label: string, value: number, onChange: (next: number) => void) => (
        <Stack align='center' gap={4} style={{ flexShrink: 0 }}>
            ...
            <NumberStepper aria-label={label} value={value} min={0} max={MEAL_SLOT_RANGE_MAX} onChange={next => onChange(next ?? 0)} style={{ width: 124 }} />
        </Stack>
    );
    ...
    const mealRangeModal = <Modal
        ...
        bodyStyle={{ background: '#f8fafc' }}
    >
        <Stack direction='column' align='stretch' gap={10} fullwidth>
            <Typography.Text type='secondary' style={{ display: 'block', fontSize: 12, lineHeight: '18px' }}>
            ...
            <Box style={{ border: '1px solid rgba(15,23,42,0.08)', borderRadius: 8, background: '#fff', padding: 12 }}>
                <Stack justify='space-between' align='center' gap={8} wrap='wrap' style={{ width: '100%', marginBottom: 8 }}>
                    ...
                    <ActionButton icon={<SaveOutlined />} onClick={_saveSmartPlannerTemplate}>Lưu thành mẫu</ActionButton>
                </Stack>
                {smartPlannerTemplates.length === 0
                    ...
                    : <Collapse
                        ...
                        items={smartPlannerTemplates.map(template => ({
                            ...
                            label: <Stack justify='space-between' align='center' gap={6} wrap='wrap' style={{ width: '100%' }}>
                                ...
                                <Stack align='center' gap={6} style={{ flexShrink: 0 }}>
                                    <ActionButton
                                        ...
                                        onClick={event => { event.stopPropagation(); _applySmartPlannerTemplate(template); }}
                                    >Áp dụng</ActionButton>
                                    <ActionButton
                                        shape='circle'
                            ...
                            children: renderTemplateDetail(template),
                        }))}
                    />}
            </Box>
            {mealSlots.map(slot => {
                ...
                return <Box key={slot} style={{ border: `1px solid ${meta.border}`, borderRadius: 8, background: '#fff', padding: 12 }}>
                    <Stack justify='space-between' align='center' gap={10} wrap='wrap' style={{ width: '100%' }}>
                        ...
                        <Stack align='center' gap={10} style={{ flexShrink: 0 }}>
                            {renderStepperControl('Min', range.min, next => _updateMealSlotDishRange(slot, 'min', next))}
                            {renderStepperControl('Max', range.max, next => _updateMealSlotDishRange(slot, 'max', next))}
                        </Stack>
                    </Stack>
                            ...
                            children: tagRequirementOptions.length === 0 ? <Typography.Text type='secondary' style={{ display: 'block', fontSize: 11, lineHeight: '16px' }}>Chưa có món nào gắn nhãn. Thêm tag cho món trong trang Món ăn để dùng tính năng này.</Typography.Text> : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 6 }}>
                                {tagRequirementOptions.map(tag => {
                                    ...
                                    return <Stack key={tag} justify='space-between' align='center' gap={6} style={{ padding: '6px 8px', borderRadius: 6, border: `1px solid ${active ? 'rgba(19,168,168,0.40)' : 'rgba(15,23,42,0.08)'}`, background: active ? '#e6fffb' : '#f8fafc' }}>
                                        ...
                                        <NumberStepper aria-label={`yêu cầu ${tag}`} value={value} min={0} max={MEAL_SLOT_RANGE_MAX} size='small' onChange={next => _setSlotTagRequirement(slot, tag, next ?? 0)} style={{ width: 116, flexShrink: 0 }} />
                                    </Stack>;
                                })}
                            </div>,
                        }]}
    ...
    return <Box className='smart-planner-page' data-testid='smart-meal-planner-page'>
        ...
        <div className='smart-planner-grid'>
                                ...
                                {advancedEnabled ? <>
                                    <div className='smart-planner-adv-group'>
                                        ...
                                        <div className='smart-planner-adv-grid'>
                                            <div>
                                                <PlannerFieldLabel helpKey='budget' label={<><DollarCircleOutlined /> Ngân sách mỗi ngày (đ)</>}>Ngân sách được kiểm tra trên tổng cả ngày. Sáng, trưa và tối có thể dùng số tiền khác nhau, miễn tổng chi phí ước tính của ngày phù hợp ngân sách.</PlannerFieldLabel>
                                                <NumberStepper min={0} step={10000} value={dailyBudget} onChange={value => { setDailyBudget(Number(value ?? 0)); _clearSuggestions(); }} style={{ width: '100%' }} />
                                            </div>
                                            {scope === 'week' && <div>
                                                <PlannerFieldLabel helpKey='weekly-budget' label={<><DollarCircleOutlined /> Ngân sách tuần (đ)</>}>Nếu đặt ngân sách tuần, phần tổng kết sẽ cảnh báo khi chi phí cần mua của cả tuần vượt mức này.</PlannerFieldLabel>
                                                <NumberStepper min={0} step={50000} value={weeklyBudget} onChange={value => { setWeeklyBudget(value === null ? undefined : Number(value)); _clearSuggestions(); }} placeholder='Không giới hạn' style={{ width: '100%' }} />
                                            </div>}
                                            <div>
                                                <PlannerFieldLabel helpKey='extra-spend' label='Mua thêm tối đa (đ)'>Giới hạn tham chiếu cho phần cần mua thêm sau khi trừ tồn kho. Bật Tiết kiệm để giới hạn này có trọng lượng rõ hơn trong xếp hạng.</PlannerFieldLabel>
                                                <NumberStepper min={0} step={10000} value={maxExtraSpend} onChange={value => { setMaxExtraSpend(value === null ? undefined : Number(value)); _clearSuggestions(); }} placeholder='Không giới hạn' style={{ width: '100%' }} />
                                            </div>
                                        </div>
            ...
            <Box className='smart-planner-panel'>
                {dishes.length === 0 ? <Empty description='Chưa có món ăn để lập thực đơn' /> : isSuggesting ? <Box style={{ minHeight: 220, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                </Box> : !hasSuggested ? <Empty description='Chọn tiêu chí rồi nhấn Gợi ý' image={Empty.PRESENTED_IMAGE_SIMPLE} /> : <Stack direction='column' align='stretch' gap={12} fullwidth>
                    {planSummary && <Box className='smart-planner-summary'>
                        <div className='smart-planner-summary-head'>
                            ...
                            <ActionButton shape='circle' aria-label='Giải thích các chỉ số tổng kết' aria-expanded={openHelpKey === 'summary'} icon={<QuestionCircleOutlined />} onClick={() => _toggleHelp('summary')} style={{ color: openHelpKey === 'summary' ? '#13a8a8' : '#6b7280', flexShrink: 0 }} />
                        </div>
