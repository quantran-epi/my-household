        ...
        {mealSections.map(section => {
            ...
            return <Box key={section.key} style={{
                ...
                <Stack justify="space-between" align="center" gap={8} style={{ marginBottom: 9 }}>
                    ...
                    <ActionButton tone="primary" icon={<PlusOutlined />} onClick={() => setAddingMeal(addingMeal === section.key ? null : section.key)}>Thêm món</ActionButton>
                </Stack>
                    ...
                    {dishIds.map(dishId => {
                        ...
                        return <Box key={dishId} style={{ border: "1px solid rgba(15,23,42,0.08)", borderRadius: 8, background: "rgba(255,255,255,0.94)", padding: 9 }}>
                            <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) auto", gap: 8, alignItems: "start" }}>
                                ...
                                <ActionButton tone="danger" shape="circle" aria-label="Xóa món" icon={<DeleteOutlined />} onClick={() => _onRemoveDish(section.key, dishId)} />
                            </div>
