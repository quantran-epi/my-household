                ...
                {slotRows.map(row => <Box key={row.key} style={{ border: '1px solid rgba(15,23,42,0.08)', borderRadius: 8, background: '#fff', padding: 10 }}>
                    <Stack align='center' justify='space-between' gap={8}>
                        ...
                        <Stack align='center' gap={6}>
                            <NumberStepper min={0} max={23} value={draft[row.key].hour} onChange={value => _setClock(row.key, { hour: Number(value ?? 0) })} style={{ width: 132 }} />
                            <Typography.Text type='secondary'>:</Typography.Text>
                            <NumberStepper min={0} max={59} value={draft[row.key].minute} onChange={value => _setClock(row.key, { minute: Number(value ?? 0) })} style={{ width: 132 }} />
                            <ActionButton shape='circle' aria-label={`Giải thích ${row.label}`} icon={<QuestionCircleOutlined />} onClick={() => setHelpKey(value => value === row.key ? undefined : row.key)} style={{ color: helpKey === row.key ? '#13a8a8' : '#9ca3af' }} />
                        </Stack>
                    </Stack>
