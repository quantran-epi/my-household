...
import { CookingMealFeedbackHistoryRecord, CookingSessionMemberFeedback } from '@store/Models/CookingSession';
...
export const MemberDishFeedbackHistoryWidget: React.FC<MemberDishFeedbackHistoryWidgetProps> = ({ lockedDate, maxRows, compact }) => {
    ...
    const renderRecord = (record: CookingMealFeedbackHistoryRecord) => {
        ...
        const slot = MealFeedbackTitleHelper.getSlotLabel(record.mealSlot);
        const mealName = MealFeedbackTitleHelper.normalize(record.mealTitle, record.mealSlot) || 'Thực đơn';
        const dateLabel = dayjs(record.mealDate).format('DD/MM/YYYY');
        const subtitle = [slot, mealName === slot ? null : mealName, dateLabel].filter(Boolean).join(' · ');
