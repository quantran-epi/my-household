...
export const SyncBackupHealthScreen = () => {
    ...
    usePageActions([
        { key: 'refresh-health', label: 'Làm mới', icon: <ReloadOutlined />, onClick: refreshHealth, disabled: refreshing },
    ], [refreshing, refreshHealth]);
    ...
    return <Box data-testid='sync-backup-health' style={pageStyle}>
        ...
        <section style={cardStyle}>
            ...
            <div style={{ padding: 12 }}>
                ...
                <Stack wrap='wrap' gap={8} style={{ marginTop: 10 }}>
                    <ActionButton icon={<CheckCircleOutlined />} loading={isTesting} disabled={!gistId || !gistToken} onClick={() => testGistConfig()}>Kiểm tra Gist</ActionButton>
                    <ActionButton tone='success' icon={<CloudUploadOutlined />} loading={isPushing} disabled={!gistId || !gistToken} onClick={_confirmPushPersonal}>Sao lưu ngay</ActionButton>
                    <ActionButton tone='primary' icon={<CloudDownloadOutlined />} loading={isPulling} disabled={!gistId || !gistToken} onClick={_confirmPullPersonal}>Khôi phục từ Gist</ActionButton>
                </Stack>
            </div>
        </section>
        ...
        <section style={cardStyle}>
            ...
            <div style={{ padding: 12 }}>
                ...
                <Stack wrap='wrap' gap={8} style={{ marginTop: 10 }}>
                    <ActionButton icon={<ReloadOutlined />} loading={isSyncChecking} onClick={() => _checkShared(false)}>Kiểm tra cập nhật</ActionButton>
                    <ActionButton tone='warning' icon={<WarningOutlined />} loading={isSyncChecking} onClick={() => _checkShared(true)}>Force sync</ActionButton>
                </Stack>
            </div>
        </section>
