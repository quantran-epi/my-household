import { BarChartOutlined, CalendarOutlined, CheckCircleOutlined, ClockCircleOutlined, DeleteOutlined, DollarCircleOutlined, FireOutlined, MoreOutlined, RightOutlined, RollbackOutlined, ShoppingCartOutlined, WarningOutlined } from '@ant-design/icons';
...
import { LeftoverTrackerItem, discardLeftoverItem, eatLeftoverServings, finishLeftoverItem } from '@store/Reducers/AppContextReducer';
    ...
    return <Box style={{
        ...
        {activeMetric && <Box style={{ position: 'relative', marginTop: 8, borderRadius: 8, padding: 10, background: 'rgba(255,255,255,0.96)', border: '1px solid rgba(255,255,255,0.78)', boxShadow: '0 10px 22px rgba(34,17,83,0.12)' }}>
            <Stack justify='space-between' align='flex-start' gap={8}>
                ...
                <ActionButton onClick={activeMetric.onOpen} style={{ flexShrink: 0, color: activeMetric.tone, borderColor: `${activeMetric.tone}55`, fontWeight: 700 }}>
...
const ActionRow: React.FunctionComponent<{
    ...
    return <button className='dashboard-action-row' data-testid={testId} onClick={onOpen} style={{ width: '100%', border: 0, background: 'transparent', padding: 0, textAlign: 'left', cursor: 'pointer' }}>
        <div className='dashboard-action-card' style={{ border: '1px solid rgba(15,23,42,0.08)', borderRadius: 8, background: '#fff', padding: '11px 12px', boxShadow: '0 6px 16px rgba(15,23,42,0.05)', transition: 'border-color 160ms ease, box-shadow 160ms ease, transform 160ms ease', minWidth: 0 }}>
            ...
            {tags && <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 5 }}>{tags}</div>}
        </div>
    </button>;
}
...
const ShoppingListRow: React.FunctionComponent<{ item: ShoppingList; cost: string; onOpen: () => void }> = ({ item, cost, onOpen }) => {
    ...
    return <ActionRow
        ...
        title={item.name}
        description={<>{plannedLabel} · {progress.done}/{progress.total} nguyên liệu<span style={{ display: 'block', color: accent, fontWeight: 750 }}>{cost}</span></>}
        accent={accent}
...
const LeftoverRow: React.FunctionComponent<{
    ...
    onFinish: () => void;
    onDiscard: () => void;
}> = ({ item, onOpen, onEatPart, onFinish, onDiscard }) => {
    ...
    return <Box style={{ border: '1px solid rgba(15,23,42,0.08)', borderRadius: 8, background: '#fff', padding: '11px 12px', boxShadow: '0 6px 16px rgba(15,23,42,0.05)', minWidth: 0 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'auto minmax(0, 1fr) auto', gap: 9, alignItems: 'start' }}>
            ...
            <Stack align='center' gap={6} style={{ flexShrink: 0 }}>
                <Tag color={daysLeft !== null && daysLeft < 0 ? 'red' : daysLeft !== null && daysLeft <= 1 ? 'orange' : 'green'} style={{ marginInlineEnd: 0 }}>{item.portions} phần</Tag>
                <Dropdown menu={{
                    items: [
                        { label: 'Ăn một phần', key: 'eat_part', icon: <RollbackOutlined /> },
                        { label: 'Đã hết', key: 'finish', icon: <CheckCircleOutlined /> },
                        { label: 'Bỏ', key: 'discard', icon: <DeleteOutlined />, danger: true },
                    ],
                    onClick: ({ key }) => {
                        if (key === 'eat_part') onEatPart();
                        if (key === 'finish') onFinish();
                        if (key === 'discard') onDiscard();
                    },
                }} placement='bottomRight'>
                    <Button aria-label='Thao tác món còn lại' type='text' icon={<MoreOutlined />} style={{ width: 32, height: 32, paddingInline: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }} />
                </Dropdown>
            </Stack>
        </div>
    </Box>;
...
export const DashboardScreen = () => {
    ...
    const [eatPartItem, setEatPartItem] = React.useState<LeftoverTrackerItem | null>(null);
    const [eatPartCount, setEatPartCount] = React.useState<number>(1);
    const [discardItem, setDiscardItem] = React.useState<LeftoverTrackerItem | null>(null);
    const [discardReason, setDiscardReason] = React.useState<string>('');
    useScreenTitle({ value: 'Tổng quan', deps: [] });

    const openAnalytics = React.useCallback(() => openRoute(RootRoutes.AuthorizedRoutes.Analytics()), [openRoute]);
    usePageActions([
        { key: 'open-analytics', label: 'Phân tích', icon: <BarChartOutlined />, onClick: openAnalytics },
    ], [openAnalytics]);
    ...
    const heroMetrics: DashboardHeroMetric[] = [
        ...
        },
        {
            ...
            actionLabel: 'Mở phần còn lại',
            onOpen: () => openRoute(RootRoutes.AuthorizedRoutes.ScheduledMealRoutes.Leftovers()),
        },
        {
            key: 'inventory',
            label: 'Kho',
    ...
    return <Box data-testid="dashboard" style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: '0 0 14px', maxWidth: 760, margin: '0 auto' }}>
        ...
        <Section
            ...
            tone='#13a8a8'
            action={<ActionButton onClick={() => openRoute(RootRoutes.AuthorizedRoutes.ScheduledMealRoutes.PrepTasks())}>Mở</ActionButton>}
        >
        ...
        <Section
            ...
            icon={<CalendarOutlined />}
            tone='#1677ff'
            action={<ActionButton onClick={() => openRoute(RootRoutes.AuthorizedRoutes.ScheduledMealRoutes.List())}>Mở thực đơn</ActionButton>}
        >
        ...
        <Section
            ...
            icon={<FireOutlined />}
            tone='#389e0d'
            action={<ActionButton onClick={() => openRoute(RootRoutes.AuthorizedRoutes.DishesRoutes.List())}>Mở món ăn</ActionButton>}
        >
        ...
        <Section
            ...
            icon={<WarningOutlined />}
            tone={expiredCount > 0 ? '#cf1322' : '#fa8c16'}
            action={<ActionButton onClick={() => openRoute(RootRoutes.AuthorizedRoutes.IngredientRoutes.List())}>Mở kho</ActionButton>}
        >
        ...
        <Section
            ...
            icon={<ShoppingCartOutlined />}
            tone='#0958d9'
            action={<ActionButton onClick={() => openRoute(RootRoutes.AuthorizedRoutes.ShoppingListRoutes.List())}>Mở mua sắm</ActionButton>}
        >
