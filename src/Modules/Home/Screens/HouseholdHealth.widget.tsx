import { DeleteOutlined, DownOutlined, EditOutlined, HeartOutlined, MoreOutlined, PlusOutlined, RightOutlined, SaveOutlined } from '@ant-design/icons';
...
import { DatePicker, Empty, Input, Select, Segmented, Tooltip } from 'antd';
...
export const HouseholdHealthWidget: React.FC<{ member: HouseholdMemberProfile }> = ({ member }) => {
    ...
    const _confirmRemoveRecord = (record: HouseholdHealthRecord) => {
        Modal.confirm({
    ...
    const _renderHistoryActions = (record: HouseholdHealthRecord) => <div className='household-health-history-actions'>
        <Dropdown menu={{
            items: [
                { label: 'Sửa ghi nhận', key: 'edit', icon: <EditOutlined /> },
                ...(record.type === 'sickness' ? [{ label: 'Thêm điều trị', key: 'add-treatment', icon: <PlusOutlined /> }] : []),
                { type: 'divider' },
                { label: 'Xóa ghi nhận', key: 'delete', icon: <DeleteOutlined />, danger: true },
            ],
            onClick: ({ key }) => {
                if (key === 'edit') _openRecordModal(record);
                if (key === 'add-treatment') _openTreatmentForSickness(record);
                if (key === 'delete') _confirmRemoveRecord(record);
            },
        }} placement='bottomRight' trigger={['click']}>
            <Tooltip title='Thao tác ghi nhận'>
                <Button aria-label={`Thao tác cho ${record.title}`} type='text' icon={<MoreOutlined />} style={iconButtonStyle} />
            </Tooltip>
        </Dropdown>
    </div>;
        ...
        <Box style={{ border: '1px solid rgba(15,23,42,0.08)', borderRadius: 8, background: '#fff', padding: 12 }}>
            ...
            <div style={fieldListStyle}>
                ...
                <div style={fieldRowStyle}>
                    ...
                    <NumberStepper min={30} max={260} step={1} value={draft.heightCm} onChange={value => setDraft(current => ({ ...current, heightCm: value === null ? undefined : Number(value) }))} style={{ width: '100%' }} />
                </div>
                <div style={fieldRowStyle}>
                    <Typography.Text strong style={labelStyle}>Cân nặng</Typography.Text>
                    <NumberStepper min={1} max={500} step={0.1} precision={1} value={draft.weightKg} onChange={value => setDraft(current => ({ ...current, weightKg: value === null ? undefined : Number(value) }))} style={{ width: '100%' }} />
                </div>
                <div style={fieldRowStyle}>
