...
const templateBodyActionRowStyle: React.CSSProperties = {
    display: 'flex',
...
const TEMPLATE_ITEM_ACTION_SIZE = 32;

const templateMenuButtonStyle: React.CSSProperties = {
    width: TEMPLATE_ITEM_ACTION_SIZE,
    height: TEMPLATE_ITEM_ACTION_SIZE,
    paddingInline: 0,
    borderRadius: 999,
    border: '1px solid rgba(116,54,220,0.12)',
    background: '#fbf9ff',
...
export const TemplatesScreen = () => {
    ...
    usePageActions([
        { key: 'create-meal-template', label: 'Tạo mẫu thực đơn', icon: <PlusOutlined />, onClick: _openMealCreator },
        { key: 'create-shopping-template', label: 'Tạo mẫu mua sắm', icon: <PlusOutlined />, onClick: _openShoppingCreator },
    ], []);
    ...
    return <Box data-testid='templates-screen' style={pageStyle}>
        <section style={sectionStyle}>
            ...
            <div style={bodyStyle}>
                <div style={templateBodyActionRowStyle}>
                    <Button type='primary' icon={<PlusOutlined />} aria-label='Tạo mẫu thực đơn' onClick={_openMealCreator}>Tạo mẫu</Button>
                </div>
                {weeklyMealTemplates.length === 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description='Chưa có mẫu thực đơn' /> : <Stack direction='column' align='stretch' gap={8}>
                        ...
                        return <Box key={template.id} style={templateCardStyle}>
                            <div style={templateCardInnerStyle}>
                                ...
                                <div style={templateActionsStyle}>
                                    <ActionButton tone='primary' aria-label='Áp dụng mẫu thực đơn' icon={<PlayCircleOutlined />} shape='circle' height={TEMPLATE_ITEM_ACTION_SIZE} style={templateApplyButtonStyle} onClick={() => setTemplateApplyTarget(template)} />
                                    <Dropdown menu={{
                                        items: [
        ...
        <section style={sectionStyle}>
            ...
            <div style={bodyStyle}>
                <div style={templateBodyActionRowStyle}>
                    <Button type='primary' icon={<PlusOutlined />} aria-label='Tạo mẫu mua sắm' onClick={_openShoppingCreator}>Tạo mẫu</Button>
                </div>
                {shoppingListTemplates.length === 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description='Chưa có mẫu mua sắm' /> : <Stack direction='column' align='stretch' gap={8}>
                    {shoppingListTemplates.map(template => <Box key={template.id} style={templateCardStyle}>
                        <div style={templateCardInnerStyle}>
                            ...
                            <div style={templateActionsStyle}>
                                <ActionButton tone='primary' aria-label='Áp dụng mẫu mua sắm' icon={<PlayCircleOutlined />} shape='circle' height={TEMPLATE_ITEM_ACTION_SIZE} style={templateApplyButtonStyle} onClick={() => setShoppingApplyTarget(template)} />
                                <Dropdown menu={{
                                    items: [
                                        { label: 'Xem trước', key: 'preview', icon: <EyeOutlined /> },
                                        { label: 'Sửa mẫu', key: 'edit', icon: <EditOutlined /> },
            ...
            <DeferredModalContent active={mealCreatorOpen} minHeight={180}>
                <Box style={{ padding: '4px 0' }}>
                    ...
                    {mealTemplateCreateMode === 'scratch' && mealTemplateScope === 'week' && <Stack direction='column' align='stretch' gap={10} style={{ marginTop: 10 }}>
                        {scratchWeek.map(day => <Box key={day.offset} style={{ border: '1px solid #f0f0f0', borderRadius: 8, padding: 10, background: '#fff' }}>
