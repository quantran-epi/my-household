import { DeleteOutlined, MoreOutlined, PlusOutlined, SaveOutlined, UserOutlined } from '@ant-design/icons';
...
import { Empty, Input, Segmented, Select, Switch } from 'antd';
...
const memberToolButtonStyle: React.CSSProperties = {
    width: 36,
    height: 36,
    paddingInline: 0,
    borderRadius: 999,
...
export const HouseholdProfilesScreen: React.FC = () => {
    ...
    usePageActions([
        { key: 'add-member', label: 'Thêm thành viên', icon: <PlusOutlined />, onClick: _createMember },
    ], [members.length, selectedMemberIds]);
    ...
    const _confirmRemoveMember = (member: HouseholdMemberProfile) => {
        modal.confirm({
            title: 'Xoá hồ sơ này?',
            content: member.name,
            okText: 'Xoá',
        ...
        <div className='household-layout'>
            ...
            <Box className='household-panel household-editor-panel'>
                {!activeMember || !draftMember ? <Box style={{ minHeight: 260, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <div className='household-editor-detail-tool'>
                        <Dropdown menu={{
                            items: [
                                { label: 'Xoá hồ sơ', key: 'delete', icon: <DeleteOutlined />, danger: true },
                            ],
                            onClick: ({ key }) => { if (key === 'delete') _confirmRemoveMember(activeMember); },
                        }} placement='bottomRight' trigger={['click']}>
                            <Button aria-label={`Thao tác cho ${activeMember.name}`} type='text' icon={<MoreOutlined />} style={memberToolButtonStyle} />
                        </Dropdown>
                    </div>
                    ...
                    <div className='household-field-list'>
                        ...
                        <div className='household-field-row'>
                            ...
                            <div className='household-field-control'><NumberStepper min={0.1} max={12} step={0.1} precision={1} value={draftMember.portionPreference ?? 1} onChange={value => _updateDraftMember({ portionPreference: Number(value ?? 1) })} style={{ width: '100%' }} /></div>
                        </div>
                        <div className='household-field-row'>
