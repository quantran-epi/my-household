...
import { selectIngredientsById, selectInventoryById } from "@store/Selectors";
...
export const IngredientDetailScreen = () => {
    ...
    usePageActions(isAdmin && ingredient ? [
        { key: "edit", label: "Sửa nguyên liệu", icon: <EditOutlined />, onClick: toggleEdit.show },
    ] : [], [isAdmin, ingredient]);
    ...
    return <React.Fragment>
            ...
            <Stack direction="column" align="flex-start" gap={6}>
                ...
                <Box style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 2 }}>
                    {recipeUnits.map(unit => (
                        <span
                            key={unit}
                            style={{ padding: "2px 8px", borderRadius: 999, background: "#f5f5f5", color: "#595959", fontSize: 12 }}
                        >
        ...
        {toggleSuggester.value && <DishSuggesterScreen
